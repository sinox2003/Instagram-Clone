...
function SharePostItem({uid, chatId,sendTo, removeFromList,  isChecked }) {
    ...
    useEffect(() => {
        if (isCheked) {
            sendTo({username:userProfile?.username,uid,chatId});
        } else {
            removeFromList({username:userProfile?.username,uid, chatId});
        }
    }, [isCheked]);
        ...
        isUserLoading ?
            ...
            <VStack _hover={{ bg: switchMode('whiteAlpha.200', 'blackAlpha.100') }} onClick={handleCheck} cursor={'pointer'} px={5}>
                <HStack w='full' h={'60px'}>
                    <HStack w='full'>
                        <Avatar w='44px' height='44px' src={userProfile?.profilePicURL} />
                        <Box maxH={'44px'}>
                            <Text fontSize='sm' fontWeight='semibold' cursor={'pointer'} sx={{display: '-webkit-box', WebkitLineClamp: '1', WebkitBoxOrient: 'vertical', overflow: 'hidden'}} maxW={{base:'170px',md:'200px'}}>{userProfile?.username}</Text>
                        </Box>
                    </HStack>
                    <Spacer />
                    <Center w='24px' h='24px' borderRadius='50%' aspectRatio={1} bg={isCheked && switchMode('#F8F9F9', '#0F1419')} border={'1px '} borderColor='#DFE2E7'>
